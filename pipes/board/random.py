...
def visit(
    protoboard: List[List[ProtoTile]], x: int, y: int, visited: List[List[bool]]
) -> bool:
...
def random(height: int, width: int) -> "Board":
    protoboard = [[ProtoTile() for _ in range(0, width)] for _ in range(0, height)]
    visited = [[False] * width for _ in range(0, height)]
    visit(protoboard, 0, 0, visited)

    board = Board(
        [[rotate(Tile.from_prototile(col)) for col in row] for row in protoboard]
    )
    return board
